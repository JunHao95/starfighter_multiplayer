...
class PlayerData:
    def __init__(self):
        ...
        self.bullet_increase_timer = self.bullet_increase_timer = 0
        self.bullet_increase_delay = self.bullet_increase_delay = PLAYER_INCREASE_BULLET_DELAY
        
        self.isDead = False
        self.hasFired = False
        
        self.gameScore = 0
        self.gameEnd = False # Identifies if the game has ended
...
class Player(pygame.sprite.Sprite):
    def __init__(self, images, bullet_image, P_Prefs):
        ...
        # Define temporary multiplayer key controls
        self.multiplayerUP = pygame.K_w
        self.multiplayerDOWN = pygame.K_s
        self.multiplayerRIGHT = pygame.K_d
        self.multiplayerLEFT = pygame.K_a
        self.multiplayerFIRE = pygame.K_m
        
        self.hasFired = False
        self.hasFiredCompleted = False
        
    def getData(self):
        pData = PlayerData()
        
        pData.rect = self.rect
        pData.position = self.position
        
        pData.velocity = self.velocity
        pData.radius = self.radius
        
        pData.speed = self.speed
        pData.gun_level = self.gun_level
        pData.prev_gunlv = self.prev_gunlv
        pData.health = self.health
        pData.hasFired = self.hasFired
        pData.isDead = self.isDead
        
        pData.bullet_increase_timer = self.bullet_increase_timer
        pData.bullet_increase_delay = self.bullet_increase_delay        
        
        return pData
        
    def setData(self,playerDataIn):
        self.rect = playerDataIn.rect
        self.position = playerDataIn.position
        self.velocity = playerDataIn.velocity
        self.radius = playerDataIn.radius
        self.speed = playerDataIn.speed
        self.gun_level = playerDataIn.gun_level
        self.prev_gunlv = playerDataIn.prev_gunlv
        self.health = playerDataIn.health
        self.hasFired = playerDataIn.hasFired
        self.isDead = playerDataIn.isDead
        
        self.bullet_increase_timer = playerDataIn.bullet_increase_timer
        self.bullet_increase_delay = playerDataIn.bullet_increase_delay     
    ...
    def _move(self, keyspressed):
        ...
        if self.isMultiplayer == True:
            if keyspressed[self.multiplayerUP]:
                self.velocity.y = -self.speed
            if keyspressed[self.multiplayerDOWN]:
                self.velocity.y = self.speed
            if keyspressed[self.multiplayerLEFT]:
                self.orientation_ = self.orientations_[1]
                self.velocity.x = -self.speed
            if keyspressed[self.multiplayerRIGHT]:
                self.orientation_ = self.orientations_[2]
                self.velocity.x = self.speed
    ...
    def _shoot(self,  keyspressed):
        ...
        if now - self.shoot_timer > self.shoot_delay:
            ...
            if self.isMultiplayer == True and self.hasFired == True:
                self._play_shoot_sound()
