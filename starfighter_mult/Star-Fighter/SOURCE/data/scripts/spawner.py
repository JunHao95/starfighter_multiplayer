...
class Spawner:
    def __init__(self, player, g_diff, multiplayerModeIn):
        # Spawner defines 
        self.spawn_timer = pygame.time.get_ticks()
        
        print(str(len(player)))
        
        if multiplayerModeIn == 0:
            self.player = player[0]
            
        if multiplayerModeIn == 1:
            self.player = player[0]
            
        if multiplayerModeIn == 2:
            self.player = player[1]
            
        self.multiplayerModeIn = multiplayerModeIn
    ...
    def setPlayerFocus(self, playerIn):
    ...
    def spawn_hellfighter(self):
        ...
        if self.multiplayerModeIn == 0: # Stand-alone mode        
            e = Hellfighter(
                self.HELLFIGHTER_IMAGES,
        ...
        if self.multiplayerModeIn == 1 or self.multiplayerModeIn == 2: # Server / Client mode
            e = Hellfighter(
                self.HELLFIGHTER_IMAGES,
                self.SMALL_BULLET_IMAGE,
                Vec2(10, 10),
    ...
    def spawn_fatty(self):
        ...
        if self.multiplayerModeIn == 1 or self.multiplayerModeIn == 2:
            e = Fatty(
                self.FATTY_IMAGES,
                self.FATTY_BULLETS_IMAGES,
                Vec2(30, 10),
