...
class MultiplayerOptionsSceneMenuWidget:
    def __init__(self, P_Prefs):
        self.P_Prefs = P_Prefs
        self.is_changingkey = False
        self.user_text = ' '
        ...
        self.thisserverIP = s.getsockname()[0]
        s.close()
        
        self.clientIP = '172.0.0.1' # Loopback address by default
        ...
        x_alignment = 30.0
        y_alignment = 0.0
        self.current_moption_selected = 0 # 0 - Standalone, 1 - Server, 2 - Client
        self.current_moption_vertical = 0
        btn_x_size = 128

        # Initialize textboxes
        self.multiplayer = TextSelector(self.P_Prefs.multiplayer, MULTIPLAYER_OPTIONS, (x_alignment,y_alignment), alignment="CENTER", active=True)        
        #self.txtbox_down = Textbox(self.P_Prefs.key_down, (108,24), (x_alignment,32))
        #self.txtbox_left = Textbox(self.P_Prefs.key_left, (108,24), (x_alignment,64))
        #self.txtbox_right = Textbox(self.P_Prefs.key_right, (108,24), (x_alignment,96))
        #self.txtbox_fire = Textbox(self.P_Prefs.key_fire, (108,24), (x_alignment,128))
        ...
        if self.P_Prefs.multiplayer == STANDALONE:
            self.current_moption_selected = 0
        elif self.P_Prefs.multiplayer == SERVER_MODE:
            self.current_moption_selected = 1
        elif self.P_Prefs.multiplayer == CLIENT_MODE:
            self.current_moption_selected = 2
    ...
    def displayUserIP(self, userInput):
        self.user_text = userInput
    
    def draw(self, window):
        self.image.fill("BLACK")
        self.image.set_colorkey("BLACK")
        
        if self.current_moption_selected == 2:
            draw_text2(self.image, self.user_text, GAME_FONT, FONT_SIZE, (128+15, 32 + FONT_SIZE/2+40), "GREEN")     

        # Output server IP if required
        if self.current_moption_selected == 1:
            draw_text2(self.image, str(self.thisserverIP), GAME_FONT, FONT_SIZE, (128+15, 32 + FONT_SIZE/2+40), "GREEN")
        
        # Draw labels
        draw_text2(self.image, "MODE", GAME_FONT, FONT_SIZE, (32, 0 + FONT_SIZE/2), "WHITE")
        if (self.current_moption_selected == 1 or self.current_moption_selected == 2):        
            draw_text2(self.image, "SERVER", GAME_FONT, FONT_SIZE, (32, 32 + FONT_SIZE/2+40), "WHITE")
    ...
    def save_prefs(self):
        # Save multiplayer mode        
        if self.current_moption_selected == 0:
            self.P_Prefs.multiplayer = STANDALONE # Stand-alone
        elif self.current_moption_selected == 1:
            self.P_Prefs.multiplayer = SERVER_MODE # Server 
            self.P_Prefs.serverAddressPort = (self.thisserverIP, 20001)
        elif self.current_moption_selected == 2:
            self.P_Prefs.multiplayer = CLIENT_MODE # Client
    ...
    def select_up(self):
        ...
        if self.current_moption_vertical > 0:
            self.current_moption_vertical = self.current_moption_vertical - 1
    ...
    def select_down(self):
        ...
        if self.current_moption_vertical < 1:
            self.current_moption_vertical = self.current_moption_vertical + 1
    ...
    def select_left(self):        
        if self.current_moption_vertical == 0:
            if self.current_moption_selected == 0:
                ...
                self.current_moption_selected = self.current_moption_selected - 1
            selected_option = self.options[self.index]
            option_type = type(selected_option)
            if option_type == TextSelector:
                selected_option.go_left()
...
class OptionsSceneMenuWidget:
    def __init__(self, init_selected=0):
        ...
        # Menu
        self.options = ("VIDEO", "SOUND", "GAME", "CONTROLS", "MULTIPLAYER", "BACK")
        self.act_opt = [0 for i in range(len(self.options))] # Active options
        self.act_opt[init_selected] = 1
        self.colors = {0: "white", 1: "black"} # Colors for active/inactive menu
...
class ServerClientConnectWidget:
    def __init__(self, init_selected, P_Prefs):
        self.P_Prefs = P_Prefs
        self.is_changingkey = False
        self.user_text = ' '
    ...
    def draw(self, window):
        ...
        draw_text2(self.image, "WAITING FOR PLAYER TO CONNECT...", GAME_FONT, FONT_SIZE, (12, 100 + FONT_SIZE/2), "WHITE")

        window.blit(self.image, (0,window.get_height()*0.3))

        
# ClientScreenServerConnectWidget
class ClientScreenServerConnectWidget:
    def __init__(self, P_Prefs):
        self.P_Prefs = P_Prefs
        self.is_changingkey = False
        self.user_text = ' '
    ...
    def draw(self, window):
        ...
        draw_text2(self.image, "ATTEMPTING TO CONNECT TO SERVER...", GAME_FONT, FONT_SIZE, (12, 100 + FONT_SIZE/2), "WHITE")
